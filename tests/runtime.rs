...
#[test]
fn swap_instruction_exchanges_accumulator_and_memory() {
    let source = r#"
        .memory 4
        .init 2 17
        LOADI 23
    ...
    assert!(result.halted);
    assert_eq!(result.final_state.acc, 17);
    assert_eq!(result.final_state.memory[2], 23);
}

#[test]
fn carry_branch_instructions_follow_cmp_flag() {
    let source = r#"
        LOADI 5
        CMP 10
        JC 4
...
#[test]
fn load_and_store_pointer_follow_memory_pointer_cells() {
    let source = r#"
        .memory 6
        .init 1 4
        .init 4 17
    ...
    assert!(result.halted);
    assert_eq!(result.final_state.acc, 23);
    assert_eq!(result.final_state.memory[4], 23);
}

#[test]
fn stack_relative_frame_instructions_manage_locals() {
    let source = r#"
        .memory 8
        ADJSP -2
        LOADI 12
    ...
    assert_eq!(result.final_state.memory[6], 12);
    assert_eq!(result.final_state.memory[7], 30);
}

#[test]
fn pointer_arithmetic_reads_dereferenced_values() {
    let source = r#"
        .memory 6
        .init 1 4
        .init 4 17
    ...
    assert!(result.halted);
    assert_eq!(result.final_state.acc, 85);
}

#[test]
fn compare_pointer_uses_dereferenced_pointer_value() {
    let source = r#"
        .memory 6
        .init 1 4
        .init 4 17

...
#[test]
fn e2e_max_of_two_with_cmpm_and_jc() {
    let source = std::fs::read_to_string("programs/max_of_two.tvm").expect("fixture");
    let state = run_and_verify(&source, 32, 1);
    assert!(state.halted);
    assert_eq!(state.acc, 11);
}

#[test]
fn e2e_array_sum_with_indirect_load() {
    let source = std::fs::read_to_string("programs/array_sum_indirect.tvm").expect("fixture");
    let state = run_and_verify(&source, 128, 1);
    assert!(state.halted);
    assert_eq!(state.acc, 14);
    assert_eq!(state.memory[0], 14);
}

#[test]
fn e2e_sum_array_subroutine_with_stack_locals() {
    let source = std::fs::read_to_string("programs/sum_array_subroutine.tvm").expect("fixture");
    let state = run_and_verify(&source, 256, 2);
    assert!(state.halted);
    assert_eq!(state.acc, 14);
    assert_eq!(state.sp, 32);
}

#[test]
fn e2e_sort_four_values_with_indirect_access() {
    let source = std::fs::read_to_string("programs/bubble_sort_four.tvm").expect("fixture");
    let state = run_and_verify(&source, 512, 1);
    assert!(state.halted);
    assert_eq!(&state.memory[10..14], &[1, 2, 7, 9]);
}
...
fn e2e_each_branch_instruction() {
    ...
    // JC takes branch after CMP sets carry
    let state = run_program("LOADI 5\nCMP 10\nJC 4\nLOADI 99\nHALT\n", 16);
    assert_eq!(state.acc, -5);

    // JNC takes branch when carry is false
    let state = run_program("LOADI 10\nCMP 5\nJNC 4\nLOADI 99\nHALT\n", 16);
    assert_eq!(state.acc, 5);
}

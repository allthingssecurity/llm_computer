...
This repository explicitly credits and builds on the original open-source implementation:
...
This version extends the ISA with carry branches, pointer-based memory access, stack-relative locals, and additional example programs such as indirect array sum, GCD, recursive stack-local workflows, and bubble sort.

## Start Here If The Repo Feels Confusing
...
- what the `.tvm` program is
- what the runtime state is
- what attention is actually retrieving
...
```bash
git clone https://github.com/allthingssecurity/llm_computer && cd llm_computer
...
| `LOADR offset` | `ACC = MEM[SP + offset]` |
| `LOADP addr` | `ACC = MEM[ MEM[addr] ]` |
| `STORE addr` | `MEM[addr] = ACC` |
| `STORER offset` | `MEM[SP + offset] = ACC` |
| `SWAP addr` | `tmp = MEM[addr]; MEM[addr] = ACC; ACC = tmp` |
| `STOREP addr` | `MEM[ MEM[addr] ] = ACC` |
| `PUSH` | `SP -= 1; MEM[SP] = ACC` |
| `POP` | `ACC = MEM[SP]; SP += 1` |
| `ADD imm` | `ACC += imm` |
| `ADDM addr` | `ACC += MEM[addr]` |
| `ADDR offset` | `ACC += MEM[SP + offset]` |
| `ADDP addr` | `ACC += MEM[ MEM[addr] ]` |
| `SUB imm` | `ACC -= imm` |
| `SUBM addr` | `ACC -= MEM[addr]` |
| `SUBR offset` | `ACC -= MEM[SP + offset]` |
| `SUBP addr` | `ACC -= MEM[ MEM[addr] ]` |
| `MUL imm` | `ACC *= imm` |
| `MULM addr` | `ACC *= MEM[addr]` |
| `MULR offset` | `ACC *= MEM[SP + offset]` |
| `MULP addr` | `ACC *= MEM[ MEM[addr] ]` |
| `AND imm` | `ACC &= imm` |
...
| `CMPR offset` | `ACC = ACC - MEM[SP + offset]; carry_flag = ACC < MEM[SP + offset]` |
| `CMPP addr` | `ACC = ACC - MEM[ MEM[addr] ]; carry_flag = ACC < MEM[ MEM[addr] ]` |
...
| `JC label` | Jump if `carry_flag` |
| `JNC label` | Jump if not `carry_flag` |
| `HALT` | Stop execution |
...
**Implemented:** Compact ISA with arithmetic, memory, indirect addressing, stack-relative locals, and control flow. Transformer execution with hull-backed 2D attention. Four independent execution engines with lockstep differential verification. Vanilla STARK proofs over execution traces. Interactive TUI. CLI. Benchmarks. Property tests.

**Not implemented:** GPU acceleration. Learned/trained weights. Zero-knowledge proofs. WASM frontend. Production STWO prover integration. Full-ISA STARK AIR for the newer indirect and frame-local instructions. Formal Turing completeness: the current machine is still finite-state because `pc` and `sp` are `u8` and `acc` is `i16`.

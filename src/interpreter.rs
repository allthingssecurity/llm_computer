...
impl NativeInterpreter {
    ...
    fn stack_relative_address(&self, offset: u8) -> Result<u8> {
        checked_transition_u8("memory address", i64::from(self.state.sp) + i64::from(offset))
    }

    fn read_stack_relative(&self, offset: u8) -> Result<i16> {
        let address = self.stack_relative_address(offset)?;
    ...
    fn write_stack_relative(&mut self, offset: u8, value: i16) -> Result<()> {
        let address = self.stack_relative_address(offset)?;
    ...
    fn execute_instruction(&mut self, instruction: Instruction) -> Result<MachineState> {
        ...
        match instruction {
            ...
            Instruction::AdjustStackPointer(delta) => {
                next_state.sp = checked_transition_u8("sp", i64::from(self.state.sp) + i64::from(delta))?;
            ...
            Instruction::LoadRelative(offset) => {
                let value = self.read_stack_relative(offset)?;
                advance_with_acc(&mut next_state, next_pc, i64::from(value), false);
            }
            Instruction::LoadPointer(address) => {
                let pointer = checked_transition_u8("memory address", i64::from(self.read_memory(address)?))?;
                let value = self.read_memory(pointer)?;
                advance_with_acc(&mut next_state, next_pc, i64::from(value), false);
            }
            ...
            Instruction::StoreRelative(offset) => {
                self.write_stack_relative(offset, self.state.acc)?;
                advance(&mut next_state, next_pc);
            }
            ...
            Instruction::StorePointer(address) => {
                let pointer = checked_transition_u8("memory address", i64::from(self.read_memory(address)?))?;
                self.write_memory(pointer, self.state.acc)?;
                advance(&mut next_state, next_pc);
            }
            Instruction::Push => {
            ...
            Instruction::AddRelative(offset) => {
                let rhs = self.read_stack_relative(offset)?;
                let raw = i64::from(self.state.acc) + i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, overflowed(raw));
            }
            Instruction::AddPointer(address) => {
                let pointer =
                    checked_transition_u8("memory address", i64::from(self.read_memory(address)?))?;
                let rhs = self.read_memory(pointer)?;
                let raw = i64::from(self.state.acc) + i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, overflowed(raw));
            }
            ...
            Instruction::SubRelative(offset) => {
                let rhs = self.read_stack_relative(offset)?;
                let raw = i64::from(self.state.acc) - i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, overflowed(raw));
            }
            ...
            Instruction::MulRelative(offset) => {
                let rhs = self.read_stack_relative(offset)?;
                let raw = i64::from(self.state.acc) * i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, overflowed(raw));
            }
            ...
            Instruction::CmpRelative(offset) => {
                let rhs = self.read_stack_relative(offset)?;
                let raw = i64::from(self.state.acc) - i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, self.state.acc < rhs);
            }
            Instruction::CmpPointer(address) => {
                let pointer = checked_transition_u8(
                    "memory address",
                    i64::from(self.read_memory(address)?),
                )?;
                let rhs = self.read_memory(pointer)?;
                let raw = i64::from(self.state.acc) - i64::from(rhs);
                advance_with_acc(&mut next_state, next_pc, raw, self.state.acc < rhs);
            ...
            Instruction::JumpIfCarry(target) => jump(
                &mut next_state,
            ...
            Instruction::JumpIfNotCarry(target) => jump(
                &mut next_state,
                if self.state.carry_flag {
                    next_pc
                } else {

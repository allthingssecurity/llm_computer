    ...
    pub(crate) fn read_value(
        ...
        match memory_read {
            ...
            MemoryRead::Indirect(address) => {
                let pointer = memory.load_with_mode(address, mode)?;
                let pointer =
                    u8::try_from(pointer).map_err(|_| VmError::InvalidTransitionField {
                        field: "memory address",
                        value: i64::from(pointer),
                })?;
                memory.load_with_mode(pointer, mode).map(Some)
            }
            MemoryRead::Pointer(address) => memory.load_with_mode(address, mode).map(Some),
            MemoryRead::StackRelative(offset) => {
                ...
                memory.load_with_mode(address, mode).map(Some)
            }
...
mod tests {
    ...
    fn instruction_cases() -> Vec<Instruction> {
        vec![
            Instruction::Nop,
            Instruction::AdjustStackPointer(-2),
            Instruction::LoadImmediate(-7),
            Instruction::Load(2),
            Instruction::LoadRelative(1),
            Instruction::Store(3),
            Instruction::StoreRelative(1),
            Instruction::Push,
            Instruction::Pop,
            Instruction::AddImmediate(5),
            Instruction::AddMemory(1),
            Instruction::AddRelative(1),
            Instruction::AddPointer(1),
            Instruction::SubImmediate(4),
            Instruction::SubMemory(2),
            Instruction::SubRelative(1),
            Instruction::SubPointer(1),
            Instruction::MulImmediate(-3),
            Instruction::MulMemory(1),
            Instruction::MulRelative(1),
            Instruction::MulPointer(1),
    ...
    fn burn_feed_forward_matches_native_for_all_instruction_weights() {
        ...
        let state = MachineState {
            ...
            carry_flag: true,
            halted: false,
            memory: vec![11, 6, 33, 44, 55, 66, 77, 88],
        };
            ...
            let operand = match compiled.memory_read {
                ...
                MemoryRead::Indirect(address) => {
                    let pointer = memory
                        .load_with_mode(address, &config.attention_mode)
                        .expect("pointer read");
                    let pointer = u8::try_from(pointer).expect("valid pointer");
                    memory
                        .load_with_mode(pointer, &config.attention_mode)
                        .expect("indirect read")
                }
                MemoryRead::Pointer(address) => memory
                    .load_with_mode(address, &config.attention_mode)
                    .expect("pointer operand"),
                MemoryRead::StackRelative(offset) => {
                    let address = state.sp + offset;
                    memory
                        .load_with_mode(address, &config.attention_mode)

...
impl OnnxExecutionRuntime {
    ...
    fn resolve_operand(&self, instruction: &OnnxInstructionMetadata) -> Result<i16> {
        match instruction.memory_read {
            ...
            OnnxInstructionRead::Indirect { address } => {
                let pointer = self
                    .memory
                    .load_with_mode(address, &self.metadata.config.attention_mode)?;
                ...
                self.memory
                    .load_with_mode(pointer, &self.metadata.config.attention_mode)
            }
            OnnxInstructionRead::Pointer { address } => self
                .memory
                .load_with_mode(address, &self.metadata.config.attention_mode),
            OnnxInstructionRead::StackRelative { offset } => {
                let address = i64::from(self.state.sp) + i64::from(offset);
                let address =

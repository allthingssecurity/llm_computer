...
impl Attention2D {
    fn gather(
        ...
        let memory_value = match memory_read {
            ...
            MemoryRead::Indirect(addr) => {
                let pointer = memory.load_with_mode(addr, &self.mode)?;
                let pointer = u8::try_from(pointer).map_err(|_| VmError::InvalidTransitionField {
                    field: "memory address",
                    value: i64::from(pointer),
                })?;
                Some(memory.load_with_mode(pointer, &self.mode)?)
            }
            MemoryRead::Pointer(addr) => Some(memory.load_with_mode(addr, &self.mode)?),
            MemoryRead::StackRelative(offset) => {
                let address = i64::from(state.sp) + i64::from(offset);
                let address =
                    u8::try_from(address).map_err(|_| VmError::InvalidTransitionField {
                        field: "memory address",
                        value: address,
                    })?;
                Some(memory.load_with_mode(address, &self.mode)?)
            }
...
impl InstructionCompiler {
    ...
    pub(crate) fn compile_instruction(
        ...
        match instruction {
            ...
            Instruction::AdjustStackPointer(delta) => {
                self.compile_adjust_stack_pointer(instruction, delta)
            }
            ...
            Instruction::LoadRelative(offset) => self.compile_load_relative(instruction, offset),
            Instruction::LoadPointer(address) => self.compile_load_pointer(instruction, address),
            Instruction::Store(address) => self.compile_store(instruction, address),
            Instruction::StoreRelative(offset) => self.compile_store_relative(instruction, offset),
            Instruction::Swap(address) => self.compile_swap(instruction, address),
            Instruction::StorePointer(address) => self.compile_store_pointer(instruction, address),
            ...
            Instruction::AddRelative(offset) => self.compile_add_relative(instruction, offset),
            Instruction::AddPointer(address) => self.compile_add_pointer(instruction, address),
            ...
            Instruction::SubRelative(offset) => self.compile_sub_relative(instruction, offset),
            Instruction::SubPointer(address) => self.compile_sub_pointer(instruction, address),
            ...
            Instruction::MulRelative(offset) => self.compile_mul_relative(instruction, offset),
            Instruction::MulPointer(address) => self.compile_mul_pointer(instruction, address),
            ...
            Instruction::CmpRelative(offset) => self.compile_cmp_relative(instruction, offset),
            Instruction::CmpPointer(address) => self.compile_cmp_pointer(instruction, address),
            ...
            Instruction::JumpIfCarry(target) => self.compile_jump_if_carry(instruction, target),
            Instruction::JumpIfNotCarry(target) => {
                self.compile_jump_if_not_carry(instruction, target)
            }
    ...
    fn compile_adjust_stack_pointer(
        &self,
        instruction: Instruction,
        delta: i16,
    ) -> Result<CompiledInstruction> {
        let mut builder = self.builder();
        builder.emit_linear(OUT_NEXT_SP, 1.0, IN_SP)?;
        builder.add_output_bias(OUT_NEXT_SP, Scalar::from(delta));
        builder.emit_linear(OUT_NEXT_PC, 1.0, IN_PC_NEXT)?;
        builder.emit_linear(OUT_RAW_ACC, 1.0, IN_ACC)?;
        self.finish(
            instruction,
    ...
    fn compile_store_relative(
        ...
        builder.add_output_bias(OUT_MEM_WRITE_ADDR, Scalar::from(offset));
        builder.emit_linear(OUT_MEM_WRITE_VALUE, 1.0, IN_ACC)?;
        self.finish(
            instruction,
            MemoryRead::None,
    ...
    fn compile_store_pointer(
        ...
        builder.emit_linear(OUT_RAW_ACC, 1.0, IN_ACC)?;
        builder.add_output_bias(OUT_MEM_WRITE_ENABLE, 1.0);
        builder.emit_linear(OUT_MEM_WRITE_ADDR, 1.0, IN_OPERAND)?;
        builder.emit_linear(OUT_MEM_WRITE_VALUE, 1.0, IN_ACC)?;
        self.finish(
            instruction,
            MemoryRead::Pointer(address),
            builder,
            TransitionControls::preserve(),
        )
    }

    fn compile_swap(&self, instruction: Instruction, address: u8) -> Result<CompiledInstruction> {
        let mut builder = self.builder();
        self.preserve_sp(&mut builder)?;
        builder.emit_linear(OUT_NEXT_PC, 1.0, IN_PC_NEXT)?;
        builder.emit_linear(OUT_RAW_ACC, 1.0, IN_OPERAND)?;
    ...
    fn compile_jump_if_carry(
        ...
        self.preserve_sp(&mut builder)?;
        builder.emit_linear(OUT_NEXT_PC, 1.0, IN_PC_NEXT)?;
        builder.emit_linear(OUT_NEXT_PC, Scalar::from(target), IN_CARRY)?;
        builder.emit_product(OUT_NEXT_PC, -1.0, IN_CARRY, IN_PC_NEXT)?;
        builder.emit_linear(OUT_RAW_ACC, 1.0, IN_ACC)?;
        self.finish(
            instruction,
            MemoryRead::None,
...
mod tests {
    ...
    #[test]
    fn compile_jc_takes_branch_when_carry() {
        let config = TransformerVmConfig::default();
        let compiler = InstructionCompiler::new(&config).unwrap();
        let compiled = compiler
            .compile_instruction(Instruction::JumpIfCarry(7))
            .unwrap();
    ...
    #[test]
    fn compile_jnc_falls_through_when_carry() {
        let config = TransformerVmConfig::default();
        let compiler = InstructionCompiler::new(&config).unwrap();
        let compiled = compiler
            .compile_instruction(Instruction::JumpIfNotCarry(7))
            .unwrap();
    ...
    #[test]
    fn compile_swap_exchanges_acc_and_memory_value() {
        let config = TransformerVmConfig::default();
        let compiler = InstructionCompiler::new(&config).unwrap();
        let compiled = compiler.compile_instruction(Instruction::Swap(5)).unwrap();

        let state = MachineState {
            acc: 42,
            ..MachineState::new(8)
        };
        let transition = GatedFeedForward.apply(
            &state,
            &compiled,
            AttentionContext {
        ...
        assert_eq!(transition.acc, 99);
        assert_eq!(transition.pc, 1);
        assert_eq!(transition.sp, 8);
        assert_eq!(transition.memory_write, Some((5, 42)));
    }
    ...
    #[test]
    fn compile_load_pointer_reads_indirect_value() {
        let config = TransformerVmConfig::default();
        let compiler = InstructionCompiler::new(&config).unwrap();
        let compiled = compiler
            .compile_instruction(Instruction::LoadPointer(3))
            .unwrap();

        let state = MachineState::new(8);
        let context = AttentionContext {
            memory_value: Some(77),
        };
        let transition = GatedFeedForward.apply(&state, &compiled, context);

        assert_eq!(transition.acc, 77);
    }

    #[test]
    fn compile_store_pointer_uses_operand_as_write_address() {
        let config = TransformerVmConfig::default();
        let compiler = InstructionCompiler::new(&config).unwrap();
        let compiled = compiler
            .compile_instruction(Instruction::StorePointer(3))
            .unwrap();
